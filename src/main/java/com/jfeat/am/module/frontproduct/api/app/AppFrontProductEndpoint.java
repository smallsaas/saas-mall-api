...
/*    @Resource
    ...
    @ApiOperation(value = "FrontProduct 列表信息", response = FrontProductRecord.class)
    @GetMapping("/category/{uniqueName}")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pageNum", dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", dataType = "Integer"),
            @ApiImplicitParam(name = "search", dataType = "String"),
    ...
    public Tip queryProductsWithChild(Page<FrontProductRecord> page,
                             @PathVariable(value ="uniqueName") String uniqueName,
                             @RequestParam(name = "pageNum", required = false, defaultValue = "1") Integer pageNum,
                             @RequestParam(name = "pageSize", required = false, defaultValue = "10") Integer pageSize,
                             @RequestParam(name = "search", required = false) String search,
                             @RequestParam(name = "id", required = false) Long id,
        ...
        frontProductCategoryQueryWrapper.eq(FrontProductCategory.CATEGORY_NAME,uniqueName);
        ...
        if (frontProductCategory!=null){
            ...
            if (treeProductCategory!=null){
                List<FrontProductCategoryRecord> frontProductCategoryRecords = frontProductCategoryService.productCategoryTreeToList(treeProductCategory);
                categoryIds = frontProductCategoryRecords.stream().map(FrontProductCategoryRecord::getId).collect(Collectors.toList());
            }

            if (categoryIds!=null&&categoryIds.size()>0){
                page.setRecords(frontProductService.findProductPageByCategoryIds(page,categoryIds, record, search, orderBy, null, null,supplierId,supplierName));
            }
        }
